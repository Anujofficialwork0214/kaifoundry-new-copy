...
export default function BlogCarousel() {
  ...
  const extendedBlogs = useMemo(() => {
    if (blogs.length === 0 || blogs.length <= visibleCards) {
      return blogs;
    }
    const itemsToClone = blogs.slice(0, visibleCards);
    return [...blogs, ...itemsToClone];
  }, [blogs, visibleCards]);
  ...
  const nextSlide = useCallback(() => {
    if (!isTransitioning || blogs.length <= visibleCards) return;
    setCurrentIndex((prev) => prev + 1);
  }, [isTransitioning, blogs.length, visibleCards]);
  ...
  }, [isTransitioning, blogs.length, visibleCards, currentIndex]);
  ...
  useEffect(() => {
    ...
    if (blogs.length > visibleCards && window.innerWidth >= 768) {
      interval = setInterval(() => {
        nextSlide();
      }, 3000);
    }
  ...
  }, [blogs.length, visibleCards, nextSlide]);
  ...
  const handleTransitionEnd = () => {
    if (currentIndex >= blogs.length) {
      ...
      setCurrentIndex(currentIndex % blogs.length);
    }
  };
  ...
  const isCarouselActive = blogs.length > visibleCards;
  const activeDotIndex = isCarouselActive ? currentIndex % blogs.length : 0;
      ...
      <div className="relative">
        {isCarouselActive && (
          <>
            <button
              onClick={prevSlide}
              style={{ backgroundColor: "#BA24D5" }}
              className="absolute left-0 top-1/2 hidden md:block transform -translate-y-1/2 p-3 rounded-lg cursor-pointer text-white text-3xl hover:bg-purple-700 transition z-10"
              aria-label="Previous blog posts"
            >
              <Image src='/svg/chevonArrow.svg' alt="arrow" width={30} height={30} />
            </button>
            <button
              onClick={nextSlide}
              style={{ backgroundColor: "#BA24D5" }}
              className="absolute right-0 top-1/2 hidden md:block transform -translate-y-1/2 p-3 rounded-lg cursor-pointer text-white text-3xl hover:bg-purple-700 transition z-10"
              aria-label="Next blog posts"
            >
              <Image src='/svg/chevonArrow.svg' className="rotate-180" alt="arrow" width={30} height={30} />
            </button>
          </>
        )}
        ...
        <div className="overflow-hidden">
          {loading ? (
            ...
            <div
              ref={carouselRef}
              className="flex flex-col md:flex-row gap-8 ml-0 lg:ml-24"
              style={{
                transform: `translateX(calc(-${currentIndex} * (100% / ${visibleCards}) - ${currentIndex * 32}px))`,
            ...
            >
              {extendedBlogs.map((blog, index) => (
                <div
                  key={`${blog.id}-${index}`}
                  className="flex-shrink-0 w-full pb-4 lg:px-0 px-2"
                  style={{
                    flex: `0 0 calc((100% / ${visibleCards}) - (32px * ${visibleCards - 1} / ${visibleCards}))`
                  }}
                >
                  <div className="bg-white shadow-lg lg:shadow-md flex flex-col h-[470px] overflow-hidden transition-transform duration-300 hover:scale-105">
                    <div className="relative w-full h-[246px] bg-gray-100">
                      {!loadedImages[blog.id] && <Skeleton className="absolute inset-0" />}
                      <Image
                        src={blog.image}
                        alt={blog.title}
                        fill
